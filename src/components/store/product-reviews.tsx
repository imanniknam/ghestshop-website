  ...
  return (
    <span className="inline-flex items-center gap-0.5" aria-label={`${toPersianDigits(rating)} از ۵`}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          key={i}
          className={cn(size, i < Math.round(rating) ? 'fill-[#407DC0] text-gold' : 'fill-transparent text-foreground/25')}
          aria-hidden
        />
      ))}
    </span>
            ...
            {summary.distribution.map((count, i) => {
              ...
              return (
                <div key={starLevel} className="flex items-center gap-2">
                  <span className="w-4 shrink-0 text-sm text-foreground/50 tabular-nums">
                    {toPersianDigits(starLevel)}
                  </span>
                  <Star className="h-3 w-3 shrink-0 fill-[#407DC0]/60 text-gold/60" aria-hidden />
                  <span className="h-2 flex-1 overflow-hidden rounded-full bg-foreground/10">
                    <motion.span
                      className="block h-full rounded-full bg-gradient-to-l from-[#407DC0] to-[#38BDF8]"
                      initial={{ width: 0 }}
                  ...
                  <span className="w-8 shrink-0 text-left text-sm text-foreground/45 tabular-nums">
                    ٪{toPersianDigits(pct)}
                  </span>
                </div>
              );
            ...
            <li key={review.id} className={cn('rounded-2xl p-4', glassInset)}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-[#407DC0] to-[#38BDF8] text-sm font-black text-white">
                  ...
                  <div>
                    <p className="flex items-center gap-1.5 text-sm font-bold text-foreground">
                      {review.name}
                      {review.verified && (
                        <span className="inline-flex items-center gap-0.5 rounded-full bg-emerald-500/15 px-1.5 py-0.5 text-sm font-medium text-emerald-400">
                          <BadgeCheck className="h-3 w-3" aria-hidden />
                          خرید تأییدشده
                        </span>
                      )}
                    </p>
                    <span className="text-sm text-foreground/45">{review.dateLabel}</span>
                  </div>
                </div>
