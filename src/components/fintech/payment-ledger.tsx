...
const STATUS_THEME: Record<InstallmentStatus, StatusTheme> = {
  ...
  PENDING_REVIEW: {
    label: 'در حال بررسی',
    dot: 'bg-amber-400 text-[var(--color-on-primary)]',
  ...
  UNPAID: {
    ...
    dot: 'bg-[var(--color-primary)] text-[var(--color-on-primary)]',
    ring: 'ring-[var(--color-primary)]/30',
    card: 'border-[var(--color-primary)]/30 bg-[var(--color-primary)]/[0.06]',
    text: 'text-[var(--color-primary)]',
  },
  UPCOMING: {
    label: 'اقساط آتی',
    dot: 'bg-foreground/10 text-[var(--color-foreground)]/50',
    ring: 'ring-foreground/10',
    card: 'border-foreground/10 bg-foreground/[0.03]',
    text: 'text-[var(--color-foreground)]/45',
  },
};
...
function LedgerHeader({
  ...
  return (
    ...
    >
      <div
        aria-hidden
        className="pointer-events-none absolute -left-16 -top-16 h-44 w-44 rounded-full bg-[var(--color-primary)]/25 blur-3xl"
      />
      <div className="relative z-10 flex flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-1">
            <span className="text-xs text-[var(--color-foreground)]/60">مانده بدهی</span>
            <span className="text-2xl font-black tabular-nums text-[var(--color-foreground)]">
              {formatToman(plan.remainingBalance)}
            </span>
          </div>
          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[var(--color-primary)]/15 text-[var(--color-primary)]">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center justify-between text-xs text-[var(--color-foreground)]/60">
            ...
            <span className="font-bold text-[var(--color-foreground)]">٪{toPersianDigits(progress.pct)}</span>
          </div>
          <div className="h-2 w-full overflow-hidden rounded-full bg-foreground/10">
            <motion.div
              ...
              className="h-full rounded-full bg-gradient-to-l from-[var(--color-primary)] to-[var(--color-secondary)]"
            />
          </div>
        </div>
      </div>
...
function SummaryCell({ label, value }: { label: string; value: string }): ReactNode {
  return (
    <div className="rounded-2xl bg-foreground/5 p-3">
      <p className="text-[11px] text-[var(--color-foreground)]/50">{label}</p>
      <p className="mt-1 text-sm font-bold tabular-nums text-[var(--color-foreground)]">
        {value} <span className="text-[10px] font-normal text-[var(--color-foreground)]/40">تومان</span>
      </p>
    </div>
  );
}
    ...
    >
      <motion.div layout="position" className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-xs text-[var(--color-foreground)]/55">
            سررسید: {formatJalali(month.dueDate)}
          </span>
        </div>
        <div className="flex flex-col items-end gap-1">
          <span className={cn('rounded-full px-2.5 py-0.5 text-[11px] font-bold', theme.card, theme.text)}>
            {theme.label}
          </span>
          <span className="text-sm font-black tabular-nums text-[var(--color-foreground)]">
            {formatToman(month.amount, { withSuffix: false })}
            <span className="mr-1 text-[10px] font-normal text-[var(--color-foreground)]/45">تومان</span>
          </span>
        </div>
      </motion.div>
      ...
      {month.status === 'PAID' && month.slip && (
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 border-t border-foreground/10 pt-3 text-xs text-[var(--color-foreground)]/60">
          <span className="flex items-center gap-1">
            <BadgeCheck className="h-3.5 w-3.5 text-emerald-400" aria-hidden />
            کد پیگیری:{' '}
            <span dir="ltr" className="font-semibold tabular-nums text-[var(--color-foreground)]/80">
      ...
      {month.status === 'PENDING_REVIEW' && (
        <div className="relative mt-3 border-t border-foreground/10 pt-3">
          ...
          <div className="pointer-events-none mt-2 flex select-none items-center gap-2 opacity-60 blur-[2px]">
            <span className="rounded-lg bg-foreground/10 px-3 py-1.5 text-xs">ویرایش رسید</span>
            <span className="rounded-lg bg-foreground/10 px-3 py-1.5 text-xs">حذف رسید</span>
            <Lock className="h-3.5 w-3.5 text-[var(--color-foreground)]/50" aria-hidden />
          </div>
        </div>
      )}
      ...
      {month.status === 'OVERDUE' && (
        <div className="mt-3 flex items-center gap-2 border-t border-red-500/20 pt-3 text-xs font-semibold text-red-400">
      ...
      {actionable && (
        <motion.button
          ...
          className={cn(
            ...
            'bg-[var(--color-primary)] text-[var(--color-on-primary)] shadow-lg shadow-[var(--color-primary)]/25 hover:brightness-110',
...
function UploadTerminal({ month, reduceMotion, onClose, onSubmitSlip, onSubmitted }: UploadTerminalProps): ReactNode {
  ...
  return (
    <motion.div
      layoutId={`ledger-card-${month.id}`}
      transition={reduceMotion ? { duration: 0 } : SPRING}
      className="overflow-hidden rounded-2xl border border-[var(--color-primary)]/40 bg-[var(--color-primary)]/[0.07] p-4 backdrop-blur-md ring-1 ring-inset ring-[var(--color-primary)]/20"
    >
      <motion.div layout="position" className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          aria-label="بستن"
          disabled={submitting}
          className="cursor-pointer rounded-lg p-2 text-[var(--color-foreground)]/50 transition-colors hover:bg-foreground/10 hover:text-[var(--color-foreground)] disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)]"
        >
      ...
      >
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor={`tracking-${month.id}`} className="text-xs font-medium text-[var(--color-foreground)]/80">
            شماره پیگیری تراکنش
          </label>
          <input
            ...
            className={cn(
              'w-full rounded-xl border bg-foreground/5 px-4 py-3 text-center text-sm tracking-[0.2em] text-[var(--color-foreground)] tabular-nums',
        ...
        {file ? (
          <div className="flex items-center gap-3 rounded-xl border border-[var(--color-primary)]/40 bg-foreground/5 p-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-black/20">
              {previewUrl ? (
                ...
                <ReceiptText className="h-5 w-5 text-[var(--color-primary)]" aria-hidden />
              )}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs text-[var(--color-foreground)]/70" dir="ltr">
                {file.name}
              </p>
              <p className="text-[11px] text-[var(--color-foreground)]/45">
            ...
            <button
              ...
              disabled={submitting}
              aria-label="حذف فایل"
              className="cursor-pointer rounded-lg p-2 text-[var(--color-foreground)]/50 transition-colors hover:text-[var(--color-destructive)] disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)]"
            >
              <X className="h-4 w-4" aria-hidden />
            </button>
          </div>
          ...
          >
            <CloudUpload className="h-6 w-6 text-[var(--color-primary)]" aria-hidden />
            <p className="text-xs font-medium text-[var(--color-foreground)]/80">
        ...
        <AnimatePresence>
          {error && (
            <motion.p
              ...
              transition={SOFT_SPRING}
              className="flex items-center gap-2 rounded-lg border border-[var(--color-destructive)]/40 bg-[var(--color-destructive)]/10 px-3 py-2 text-xs font-medium text-[var(--color-destructive)]"
            >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={onClose}
            disabled={submitting}
            className="flex-1 cursor-pointer rounded-xl border border-foreground/10 py-2.5 text-sm font-semibold text-[var(--color-foreground)]/70 transition-colors hover:bg-foreground/5 disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-ring)]"
          >
            انصراف
          </button>
          <motion.button
